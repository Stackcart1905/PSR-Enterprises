...
const getClient = () => {
    ...
    if (!accountSid || !authToken || !fromWhatsAppNumber) {
        console.warn("⚠️ WhatsApp Service: Missing Twilio configuration (SID, Token, or Number).");
        return null;
    }
...
export const sendWhatsAppMessage = async (to, message) => {
    ...
    if (!client || !fromWhatsAppNumber) {
        console.warn("⚠️ WhatsApp Service: Cannot send message. Client or fromWhatsAppNumber is missing.");
        return;
    }

    const formattedTo = formatPhoneNumber(targetTo);
    if (!formattedTo) {
        console.error(`❌ WhatsApp Service: Could not format number ${targetTo}. Message aborted.`);
        return;
    }
